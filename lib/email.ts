...
export const sendInvitationEmail = async (customerEmail: string, companyName: string, token: string) => {
  if (!email) {
    console.error("Nodemailer 'from' email is not configured. Invitation email not sent.");
    return;
  }
